...
DECLARE @resultFunc int = dbo.COUNT_STUDENTS('Экономика');
print('Количество студентов на факультете Экономика: ' + cast(@resultFunc as varchar(3)));
...
declare @resultFunc int = dbo.COUNT_STUDENTS('Экономика', '1-40 01 02');
print('Количество студентов на факультете Экономика с профессией 1-40 01 02: ' + cast(@resultFunc as varchar(3)));
...
select * from dbo.FFACPUL('ЛХ', NULL);
select * from dbo.FFACPUL(NULL, 'ИСиТ');
select * from dbo.FFACPUL('ХТиТ', null);
select * from dbo.FFACPUL('Физика', 'Математика');
...
create function FACULTY_REPORT(@c int) returns @fr table
                        ( [Факультет] varchar(50), [Количество кафедр] int, [Количество групп]  int, 
                                                                 [Количество студентов] int, [Количество профессий] int )
as begin 
